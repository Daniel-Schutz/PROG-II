        ...
        while (feof(dados)==0){ //ler até acabar o arquivo
            ...
            if (titulo.tuplas == NULL){
                ...
                for (i=0; i < titulo.qtd; i++){
                    fscanf(dados, "%d %s %d/%d/%d %s", &titulo.tuplas[i].codinscricao, titulo.tuplas[i].nomecandidato, &titulo.tuplas[i].datanasc.dia, &titulo.tuplas[i].datanasc.mes, &titulo.tuplas[i].datanasc.ano, titulo.tuplas[i].tipovaga);
                }
            }
        }
    }
