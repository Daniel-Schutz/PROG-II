            ...
            for (int i=0;i<quant;i++){ //ler até acabar o arquivo
                ...
                if (curso[index].tuplas == NULL){
                    ...
                    for (int x=0; x < curso[index].qtd; x++){
                        ...
                        if(strcmp(curso[index].tuplas[x].tipovaga,"AC")==0){
                            curso[index].quantAC+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L1")==0){
                            curso[index].quantL1+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L3")==0){
                            curso[index].quantL3+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L4")==0){
                            curso[index].quantL4+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L5")==0){
                            curso[index].quantL5+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L7")==0){
                            curso[index].quantL7+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L8")==0){
                            curso[index].quantL8+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L9")==0){
                            curso[index].quantL9+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L11")==0){
                            curso[index].quantL11+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L13")==0){
                            curso[index].quantL13+=1;
                        }
                        else if(strcmp(curso[index].tuplas[x].tipovaga,"L15")==0){
...
scanf("%s",nomeArq);
    ...
    if (arq ==NULL){
        ...
        aluno = (acertos_notas*) malloc(qtdAlunos*sizeof(acertos_notas)); //alocação
        
        for (int i = 0; i < qtdAlunos; i++){
            fscanf(arq, "%d %d %d %d %d %f", &aluno[i].insc, &aluno[i].v_ling, &aluno[i].v_mat, &aluno[i].v_nat, &aluno[i].v_hum, &aluno[i].red); //leitura dos dados
            if (i == 0){
                soma[0] = aluno[i].v_ling;
                soma[1] = aluno[i].v_mat;
                soma[2] = aluno[i].v_nat;
                soma[3] = aluno[i].v_hum;
                }
            else{
                soma[0] = soma[0] + aluno[i].v_ling;
                soma[1] = soma[1] + aluno[i].v_mat;
                soma[2] = soma[2] + aluno[i].v_nat;
                soma[3] = soma[3] + aluno[i].v_hum;
            }
        }
    }
    fclose(arq);
    /*TESTEfor (int i = 0; i < qtdAlunos; i++){
            printf("%d %d %d %d %d %.2f\n", aluno[i].insc, aluno[i].v_ling, aluno[i].v_mat, aluno[i].v_nat, aluno[i].v_hum, aluno[i].red); }
    FIM DO TESTE*/
 ...
 // calculo da media multiplicando por 2 para dar o EP correto
    ...
    desvio[0] = 2*desvioPadrao( media[0]/2, qtdAlunos, aluno, 0);
    desvio[1] = 2*desvioPadrao( media[1]/2, qtdAlunos, aluno, 1);
    desvio[2] = 2*desvioPadrao( media[2]/2, qtdAlunos, aluno, 2);
    desvio[3] = 2*desvioPadrao( media[3]/2, qtdAlunos, aluno, 3);
    ...
    // calculo EP / NF
    eP_NotaFinal(aluno, curso, qtdAlunos, quant, media, desvio); //função que calcula EP e Nota Final
    

    for(int i=0;i<qtdAlunos;i++){
        if (aluno[i].insc == 554840){
            printf("ep de ling: %f  ep de mat: %f  ep de nat: %f  ep de hum: %f\n",aluno[i].EP[0],aluno[i].EP[1],aluno[i].EP[2],aluno[i].EP[3]);
            printf("Acertos dele em ling %d  em mat %d  em nat %d em hum %d\n",aluno[i].v_ling,aluno[i].v_mat,aluno[i].v_nat,aluno[i].v_hum);
            printf("%f\n",aluno[i].notaFinal);
    ...
    printf("quantidade de AC no curso 0 e %d e o codigo do curso0 e %d\n ",curso[0].quantAC,curso[0].codCurso);
    printf("Ordenando as notas\n");
    for(int i=0;i<quant;i++){
        int acumulo=0;
        int primeiro=0;
        acumulo = curso[i].quantAC;
        primeiro;
        mergesortNota(0,curso[i].quantAC,curso[i]);//ordena os candidatos dentro do curso pela nota
        primeiro=acumulo;
        acumulo+=curso[i].quantL1;
        mergesortNota(primeiro,acumulo,curso[i]);
        primeiro=acumulo;
    ...
    printf("\nnome do segundo curso %s\n",curso[1].nomeCurso);
    for(int i=0;i<curso[1].qtd;i++){
        printf("nome do aluno %s e tipo de vaga e %s e a nota e %.2f\n",curso[1].tuplas[i].nomecandidato,curso[1].tuplas[i].tipovaga,curso[1].tuplas[i].notaFinal);
    }
...
/*while(true){
       ...
       scanf("%d", &opcao);
        ...
        if (opcao==1){
            ...
            void listaGeral(quant,curso,aluno);
            ...
            mergesortAcertos(0, qtdAlunos, aluno); //ordena o struct Acerto_notas para ficar mais facil a busca pelo insc
            ...
            if (file == NULL){
                ...
                while (i < numalteracoes){ //ler todo o arquivo e alterar enquanto lê
                    ...
                    indicecodinsc = busca_binariaAcertos(qtdAlunos, aluno, inscricaoalterar); //busca
                    // poderia se fazer uma checagem if (notaantiga == aluno[indicecodinsc].red) para dps realizar a troca
                    // porém, não achei necessário, uma vez que ja temos o indice
                    aluno[indicecodinsc].red = novanota; //alteração

                    //checar
                    printf("\n O aluno de %d que tinha a nota %0.f agr tem a nota %0.f", aluno[indicecodinsc].insc, notaantiga, aluno[indicecodinsc].red); //insc referente ao struct pra comparar com o arquivo se está tudo certo, notaantiga pois a alteração ja foi feita então não teria como chamar o struct ponto nota, e por fim o ultimo referente a nota atualizada.
                    //checado
                    i++;
                }
            }
